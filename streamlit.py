...
def predict(image, model):
    ...
    with torch.no_grad():
        ...
        # predicted class index and its probability
        prob_values, predicted = torch.max(probs, 1)

    predicted_idx = predicted.item()
    predicted_class = class_names[predicted_idx]
    predicted_score = prob_values.item()  # between 0 and 1
...
if uploaded_file is not None:
    ...
    if st.button("Generate"):
        ...
        predicted_class, score = predict(image, model)

        st.success(f"Prediction: {predicted_class} (score: {score:.2%})")